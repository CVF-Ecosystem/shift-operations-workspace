# Session Memory
Human companion to [`ACTIVE_SESSION_STATE.json`](ACTIVE_SESSION_STATE.json), provider-neutral for every agent and human; details live in the handoffs. _Last updated: 2026-08-03 (Project Operations Skill SPEC)_
## Where the project is
**2026-08-03 (PROJECT-OPERATIONS-SKILL — SPEC RE_REVIEW_PASS):** exact two-file/frontmatter/metadata, R1-R8, FT-1..FT-4 and AC-01..AC-14 contracts are independently approved after a HIGH call-cardinality conflict was repaired without waiver. Live proof now requires four separate non-mocked sessions with durable one-call FT lineages, no batch/retry, and aggregate 4/4 accounting. Exact-path WORK_ORDER is next; skill source/install/BUILD/provider/FREEZE remain unauthorized.
