...
_Last updated: 2026-07-30 (Message admission and trust repair DESIGN; SPEC next; BUILD unauthorized)_

## Where the project is

**2026-07-30 (Message admission and trust repair — DESIGN):** ADR `ADR_2026-07-30_MESSAGE_ADMISSION_TRUST_REPAIR.md` resolves `MAR-INTAKE-F1..F9` by classifying `POST /messages` as an authenticated internal-user command only: `message.create` at operator minimum, principal-derived sender, fixed `INTERNAL` source, required mutable shift, and atomic message-plus-actor-bound-audit with InMemory/SQLite/PostgreSQL parity. Existing table/domain shape is a bounded internal projection with no migration and no claim of Canonical Message Contract equivalence. External ingestion is a separate later Integration Edge tranche requiring service identity, verified canonical provenance, raw-envelope ownership, dedupe/replay, mapping and shift-or-fallback routing; it must never use internal `/messages`. Next move is SPEC only. BUILD/provider/Docker/PostgreSQL authority remains absent.
