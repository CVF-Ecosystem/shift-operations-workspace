...
_Last updated: 2026-08-01 (P2-C C3b2 Amendment 1 REVIEW_PASS/pushed; partial BUILD resume)_

## Where the project is
**2026-08-01 (P2-C C3b2 — AMENDMENT 1 REVIEW_PASS / PUSHED):** focused reproduction of the partial BUILD blocker returned `1 failed / 15 passed` in the omitted `test_handover_live_evidence_runner.py`; fixed version defaults prevented the ready-handover/no-report case reaching its Report gate. Amendment package through `3dcc966` adds exactly that path (82→83), requires explicit response-derived versions and removes permissive runner defaults. Historical SPEC Amendments 8/9 were verified restored byte-identical; new C3b2 authority is Amendment 10. Resume after this four-surface checkpoint is pushed; preserve partial unstaged BUILD, no provider call/Claude CLI/MCP, and return `READY_FOR_INDEPENDENT_P2C_C3B2_BUILD_REVIEW`.
