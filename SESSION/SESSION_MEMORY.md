# Session Memory
Human companion to [`ACTIVE_SESSION_STATE.json`](ACTIVE_SESSION_STATE.json), provider-neutral for every agent and human; details live in the handoffs. _Last updated: 2026-08-03 (Project Operations Skill review repair amendment)_
## Where the project is
**2026-08-03 (PROJECT-OPERATIONS-SKILL — AMENDMENT 1 AUTHORIZATION RE_REVIEW_PASS):** F1-F5 invalidate original four-call bundle. Amendment repair adds structural canaries, pinned byte preservation, durable dispatch, exact integrity/types/preflight and max four replacement lineages; all amendment findings closed without waiver. Governance-only push then separate repair-resume checkpoint and G6-R are next; no repair/call yet.
