...
_Last updated: 2026-07-23 (P1B-OPERATIONS-DOMAIN-EXTRACTION: WORK_ORDER approved, pre-BUILD)_
...
**2026-07-23 (P1B-OPERATIONS-DOMAIN-EXTRACTION — WORK_ORDER đã duyệt, CHƯA
BUILD):** operator đã giải quyết continuity drift bên dưới bằng cách chọn thứ
tự lane: **1) P1-B** tách domain models, 2) reconciliation
`known-principals.yaml` ↔ `users` (High Finding #4), 3) P2-A còn lại
(incidents/handovers), 4) P2-C frontend. **Chỉ lane 1 được mở**; lane 2–4 không
được triển khai, không được scope. Chain chạy INTAKE → DESIGN → SPEC →
WORK_ORDER và **dừng ngay trước BUILD**. Authorization artifacts commit riêng
tại **C1 `3e3df42`** (đúng 3 file ADR/SPEC/WORK_ORDER, 1436 dòng thêm, **không
một file implementation nào**) — đây chính là bằng chứng trong commit graph
rằng DESIGN/SPEC/WORK_ORDER đi trước BUILD, thứ mà `cd36b27` đã thiếu. Hai vòng
review độc lập trả `REVIEW_CHANGES_REQUIRED`: vòng 1 (F1–F4) bắt thiếu
`tests/unit/test_state_machine.py` trong allowlist, AC-04 kiểm identity sai
module pair, rollback rehearsal dùng scratch branch và so với `ed3d944` thay vì
`C3_PARENT`, và số liệu inventory sai; vòng 2 (F5–F6) bắt AC-01 thay sạch
`sys.path` (khiến `pydantic` không import được → phép thử vô nghĩa) và AC-06
dùng `model_validate` trên chuỗi JSON cộng "byte-identical" không có định
nghĩa. Cả 6 đã sửa trong artifacts **trước khi** operator phê chuẩn WORK_ORDER
**nguyên vẹn**. Gate: G1/G2/G3 PASS, G4 = commit continuity này, G5 (chuyển vai
IMPLEMENTATION_WORKER) và G6 (verify lại clean start) còn treo.
**BUILD chưa bắt đầu; P1-B CHƯA hoàn thành.** Chi tiết:
...
Tranche đang mở: **`P1B-OPERATIONS-DOMAIN-EXTRACTION`**, đứng ở WORK_ORDER đã
được operator phê chuẩn, **ngay trước BUILD**.

1. Codex review và commit **C2** (handoff + 3 file continuity này) với vai
   COMMIT_STEWARD.
2. Claude tuyên bố chuyển vai sang **IMPLEMENTATION_WORKER** (G5), verify lại
   clean start (G6), rồi chạy **C3** đúng trong allowlist của WORK_ORDER
   (43 path; `packages/operations-ledger/**` phải **zero-line diff**).
3. Codex review C3 độc lập bằng cách **chạy lại** từng acceptance criterion —
   không tin số liệu được báo cáo — rồi commit.
4. **C4** (catalog/roadmap/status/continuity closure) chỉ ở FREEZE, chỉ từ
   source truth quan sát được, và chỉ khi được authorize.

Không bắt đầu BUILD từ loose chat instruction. Không mở lane 2–4.
...
**P1-B (WORK_ORDER approved, pre-BUILD 2026-07-23):** **không tuyên bố P1-B đã
hoàn thành** — BUILD chưa chạy; `operations-domain` vẫn là `stub` cho tới khi
BUILD xong và reviewer độc lập chạy lại toàn bộ AC, và khi đổi thì chỉ được
lên `partial`, **không bao giờ** `enforced` (incidents/handovers/reports/
approvals vẫn chưa có model). Không tuyên bố Phase 1 exit gate đã đạt hay
PostgreSQL đã verify live — tick một roadmap item không phải là phase gate.
Không mở lane 2–4 khi P1-B còn mở. Không đụng path ngoài allowlist —
`packages/operations-ledger/**` phải zero-line diff (seam `SqlLedger(models=…)`
cố ý không refactor). **Không di chuyển `User`** sang operations-domain — nó
thuộc auth boundary, thuộc quyền quyết định của lane 2. Không gộp C3 (BUILD)
với C4 (closure) trong một commit. Không mở lại tranh luận về lane drift —
operator đã quyết.
