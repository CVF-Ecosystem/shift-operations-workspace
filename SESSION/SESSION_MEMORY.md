...
_Last updated: 2026-07-31 (P2-C C3a2 Work Order REVIEW_PASS/PUSHED; pre-BUILD G6 next)_

## Where the project is
**2026-07-31 (P2-C C3a2 — WORK ORDER REVIEW_PASS / PUSHED):** exact 79-path authorization is pushed at `fd1c09e` after independent review found and closed F1 (four omitted legacy live runners) and F2 (AC-29 rehearsal not operationalized), both without waiver. This separate four-surface commit becomes the exact pre-BUILD parent after push; root Codex may act as `IMPLEMENTATION_WORKER` only after fresh G6, while independent BUILD review remains separate. No Claude CLI, no current BUILD/provider call, and C3b-d remain blocked. C3a1 stays REVIEW_PASS/pushed at `ec90c78` under its exact single-workspace foundation boundary.
