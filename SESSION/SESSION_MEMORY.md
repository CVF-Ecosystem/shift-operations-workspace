# Session Memory
Human companion to [`ACTIVE_SESSION_STATE.json`](ACTIVE_SESSION_STATE.json), provider-neutral for every agent and human; details live in the handoffs. _Last updated: 2026-08-02 (Project Operations Skill INTAKE)_
## Where the project is
**2026-08-02 (PROJECT-OPERATIONS-SKILL — INTAKE):** automatic queue item 1 opened after Phase 2 C4 `0a29192`. INTAKE records five concrete usage examples and bounds the future artifact as a concise provider-neutral guide over canonical project truth—not a new runtime/policy engine or second truth source. Risk is R2 because the skill may steer future governance workflow. DESIGN must decide portable repo-owned source versus any separately-authorized installation target, reference routing, existing-script reuse and validation/forward tests. No skill implementation, installation, provider call, external write, commit automation, self-review or later queue item is authorized.
