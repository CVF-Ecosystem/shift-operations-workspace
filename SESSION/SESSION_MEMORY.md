...
**2026-08-01 (P2-C C3a2 — AMENDMENT 2 REVIEW_PASS / PUSHED):** the resumed partial BUILD remained at 58 in-ceiling paths and zero staged. Fresh full non-live evidence reached 1125 passed/2 failed/112 skipped; both failures were confined to the omitted `tests/integration/test_handover_live_evidence_runner.py`, whose two P2-R scenarios mint test-local principals without ACTIVE assignments. Independent review confirmed that exactly one added host is necessary/sufficient and feasible at 249 lines. Amendment 2 `55e6ae7` raises the exact ceiling `81 → 82`, forbids implicit runner assignment/production bypass, and is pushed without waiver. This separate four-surface checkpoint becomes the exact resume/review parent after push; worker reconfirms G6 and resumes only C3a2. No Claude CLI/provider call/BUILD stage/commit/self-review/FREEZE; C3b-d remain blocked.
