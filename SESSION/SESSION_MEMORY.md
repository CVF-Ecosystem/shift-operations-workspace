...
_Last updated: 2026-07-30 (Message admission WORK_ORDER REVIEW_PASS; C1/C2/G6 next; BUILD unauthorized)_

## Where the project is

**2026-07-30 (Message admission and trust repair — WORK_ORDER REVIEW_PASS):** SPEC R1-R19/AC-01..AC-23 and exact 29-path C3 Work Order are reviewed after `MAR-SPEC-REV-F1..F3` and `MAR-WO-AUTH-F1..F3` closed without waiver. Internal `/messages` requires JWT, operator permission, principal-derived sender/fixed source, atomic message/audit, copy/duplicate/evidence parity and SQLite/PostgreSQL reconnect proof; external Integration Edge remains zero-diff/later. Next is zero-BUILD C1, separate four-path C2, then G6 and assignment of an independent implementation worker. BUILD/provider/Docker/PostgreSQL remain unauthorized.
