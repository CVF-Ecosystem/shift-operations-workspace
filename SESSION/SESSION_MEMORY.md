...
_Last updated: 2026-07-30 (P2-R C2 PRE-G6; external manual-prompt worker; exact 59 paths; no Claude CLI)_

## Where the project is
**2026-07-30 (P2-R operational Report + freeze prerequisite — C2 PRE-G6):** operator-delegated authorization `32c0dc5` and sequencing Amendment 1 `312b914` are pushed. External implementation worker is assigned only through a prompt transferred manually by the operator; Codex/agent must not invoke or control Claude through CLI. Worker rehydrates from clean pushed C2, runs G6 before its first edit, then may change exactly the 59 Work Order paths and return `READY_FOR_INDEPENDENT_P2R_BUILD_REVIEW`; it cannot stage/commit/push/review/FREEZE. Current agent remains independent reviewer/commit steward. P2-C/P2-D/P5-A remain unopened.
