...
_Last updated: 2026-08-02 (P2-D pre-BUILD checkpoint; G6 required)_
## Where the project is
**2026-08-02:** P2-D offline/realtime authorization package `7437b70` is pushed with independent `REVIEW_PASS / APPROVED` and exact 49-path ceiling. This separate continuity checkpoint pins it as the BUILD parent. BUILD/provider calls have not started; the receiving `IMPLEMENTATION_WORKER` must rehydrate and pass fresh G6 before source edit. The bounded design permits only navigation fallback, actor-bound bounded staging for three CAS transitions, per-tab fail-stop replay and authenticated foreground polling; P2-D/full-shift exit/Phase 2 remain open. P2-C remains `FREEZE / CLOSED_BOUNDED` at C3d `e120a7f` plus C4 `1f3646a`.
