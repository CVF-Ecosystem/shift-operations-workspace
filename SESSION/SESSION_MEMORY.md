...
_Last updated: 2026-08-01 (P2-C C3b1 Amendment 1 approved; full G6 rerun next)_

## Where the project is
**2026-08-01 (P2-C C3b1 — AMENDMENT 1 RESUME):** C3a2 remains `FREEZE/CLOSED_BOUNDED` at `95b66b1`; C3b1 authorization `5f72a03` fixes the feasibility split and exact 34-path ceiling. Initial G6 correctly stopped because the Work Order's pnpm test command was invalid even though the canonical command independently returned 22 passed. Amendment 1 + independent `REVIEW_PASS` are pushed at `edb9b02`, change no BUILD path/claim and require the entire G6 from scratch. This four-surface resume becomes the new exact BUILD parent after push; prompt transfer stays manual, no Claude CLI/MCP/provider call/worker stage/commit/push/self-review/FREEZE, and C3b2-d remain blocked.
