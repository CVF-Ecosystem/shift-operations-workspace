# Session Memory
Human companion to [`ACTIVE_SESSION_STATE.json`](ACTIVE_SESSION_STATE.json), provider-neutral for every agent and human; details live in the handoffs. _Last updated: 2026-08-03 (Project Operations Skill Amendment 3 authorization review)_
## Where the project is
**2026-08-03 (PROJECT-OPERATIONS-SKILL — AMENDMENT 3 REPAIR RESUME):** authorization `777dbf1` is pushed and this separate checkpoint transfers the same eight unstaged BUILD paths to REPAIR_WORKER. G6-R3 must first confirm the exact v3 pins/history, zero staged/runtime residue and exact paths. Only PASS permits structured-semantic/v4 source repair; migration and provider calls remain separately gated, with a new human R2 acknowledgment required before network.
