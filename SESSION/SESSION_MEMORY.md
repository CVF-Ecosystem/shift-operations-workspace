...
_Last updated: 2026-08-01 (P2-C C3b2 Work Order REVIEW_PASS/pushed; pre-BUILD G6 next)_

## Where the project is
**2026-08-01 (P2-C C3b2 — WORK ORDER AUTHORIZED / PUSHED):** authorization `b9a62a3` contains the exact 82-path CustomerRequest-version/mutation-precondition Work Order and its independent `REVIEW_PASS`. Direct service boundaries, atomic compare order, Report/freeze semantics, feature-owned persistence splits and the live target registry are explicit; no provider call is authorized. This separate four-surface commit is the pre-BUILD parent after push. Run G6 from clean `HEAD == origin/main`; only then manually transfer the prompt. Worker never stages/commits/pushes/self-reviews/FREEZEs and returns `READY_FOR_INDEPENDENT_P2C_C3B2_BUILD_REVIEW`; C3c/d remain blocked and no Claude CLI/MCP is allowed.
