...
_Last updated: 2026-08-02 (P2-C C3c Work Order REVIEW_PASS/pushed; pre-BUILD/G6 next)_
## Where the project is
**2026-08-02 (P2-C C3c — WORK ORDER REVIEW_PASS / PRE-BUILD):** exact 38-path operator UI Work Order/review are pushed at `fbb1d31`; browser-pin Amendment 1/review close the pre-G6 version gap without waiver at `4937ac9` and require exact Playwright `1.62.1`. This renewed four-surface commit becomes the clean BUILD parent after push. Run G6, then manual prompt transfer only/no Claude CLI/MCP. Worker changes exactly 38 paths and never stages/commits/pushes/self-reviews/FREEZEs. C3d/P2-D/Phase 2 remain blocked; no provider or production-PostgreSQL claim.
