...
_Last updated: 2026-07-30 (P2-R operational Report INTAKE; DESIGN next; BUILD unauthorized)_

## Where the project is
**2026-07-30 (P2-R operational Report + freeze prerequisite — INTAKE):** fresh tranche `P2R-OPERATIONAL-REPORT-FREEZE-PREREQUISITE-2026-07-30` follows the operator-selected Phase 2 order. Migration 002 already has a reports table, but operations-domain/Ledger/workspace-api have no Report runtime vertical; the loose JSON contract differs from the migration; approval receipts have no Report action; and ShiftService still treats `report_approved` as unimplemented, allowing freeze only through an explicit audited override. Intake records `P2R-INTAKE-F1..F9`: canonical shape, lifecycle/immutability, snapshot provenance, approval authority, atomic freeze binding, backend parity, HTTP failures, compatibility/history and live evidence. Next move is DESIGN only. P2-C/P2-D/P5-A remain unopened; no BUILD/provider/Docker/PostgreSQL/stage/commit/push authority exists.
