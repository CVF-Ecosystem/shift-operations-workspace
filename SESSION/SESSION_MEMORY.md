# Session Memory
Human companion to [`ACTIVE_SESSION_STATE.json`](ACTIVE_SESSION_STATE.json), provider-neutral for every agent and human; details live in the handoffs. _Last updated: 2026-08-03 (Project Operations Skill DESIGN)_
## Where the project is
**2026-08-03 (PROJECT-OPERATIONS-SKILL — DESIGN REVIEW_PASS):** independent review accepted D1-D7 with no waiver. ADR chooses portable repo-owned source `skills/operate-shift-workspace/`; installation outside the workspace is a separate post-FREEZE action requiring explicit authority. Initial shape is only `SKILL.md` + generated `agents/openai.yaml`; dynamic current-truth routing reuses existing validators and never copies canonical truth/scripts. Validation layers are quick validation, static contracts, independent fresh-agent forward tests and sanitized real-provider evidence only for a bounded behavior claim. SPEC is next; BUILD/install/provider use remain unauthorized.
