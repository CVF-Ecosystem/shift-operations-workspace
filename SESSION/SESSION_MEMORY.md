...
_Last updated: 2026-07-30 (Message admission Amendment 2 pushed; repair authorized)_

## Where the project is

**2026-07-30 (Message admission — REVIEW_CHANGES_REQUIRED / REPAIR AUTHORIZED):** independent review found `MAR-BUILD-REV-F1..F5`: focused 106 passed but full 782/76/1 failed; invalid endpoint-port secret escaped sanitization; refusal evidence ignored audit delta; rollback/PostgreSQL assertions and receipt/catalog truth were incomplete. Amendment 2 `8d5c085` is pushed, adds exactly `tests/unit/test_shift_create_openapi_contract.py` as path 30, and requires fresh PostgreSQL/provider evidence after full non-live green. Claude Code `2.1.215` may resume only as bounded REPAIR_WORKER; no stage/commit/push/review/FREEZE. Codex remains independent reviewer/commit steward.
