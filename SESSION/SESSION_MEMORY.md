...
**2026-08-03 (P3-A REFINERY — FRESH INTAKE):** after Project Knowledge Pack closure `107c8fa`, only P3-A INTAKE is active. `refinery-bridge` is contract-only; its contract omits quarantine/provenance/data-quality/fallback results, `data_scope` has no runtime caller/minimization proof, and fixture `11h40 → 23:40` invents unsupported time semantics. Proposed scope is a local deterministic fail-closed, non-truth-owning boundary with typed source linkage, ambiguity, dedupe, sensitivity/redaction, quarantine, quality and fallback receipts. Eight design decisions remain open. No provider, DESIGN/BUILD, remote ingest/persistence, P3-B/P3-C, retrieval/RAG/learning or production authority.
