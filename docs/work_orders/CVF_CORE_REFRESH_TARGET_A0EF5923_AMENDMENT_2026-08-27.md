# Work Order Amendment — CVF Core Target `a0ef5923`
...
- Operator authority: on `2026-08-27`, the operator explicitly approved
  **`đồng ý target a0ef5923`**.

## 1. Purpose and precedence

This amendment repairs the Core-freshness prerequisite opened as
`P4D-FINAL-REV-F2`. It authorizes exactly one refresh execution from the
current project pin
`9c01832930226f2f770eafa346e01279160f22cb` to the exact target above, then
one independent completion review and one bounded Core-refresh commit/push.

All authority for prior targets and attempts, including `864c4e0...` and the
later consumed `9c018329...` target authority, is historical and cannot be
reused. This amendment supersedes only target-dependent values, the narrowed
project changed-set ceiling, and commit ownership stated here. It inherits the
accepted target-neutral bootstrap-native architecture and acceptance contract:
...
is required: the accepted architecture is target-neutral, while this exact
operator-approved target is frozen here for one execution.
...
- P4-D's implementation/review authors must not self-approve this refresh.
...
1. `.cvf/manifest.json`
2. `AGENTS.md`
3. `docs/work_orders/CVF_CORE_REFRESH_TARGET_A0EF5923_AMENDMENT_2026-08-27.md`
4. `docs/decisions/CVF_CORE_REFRESH_TARGET_A0EF5923_AUTHORIZATION_REVIEW_2026-08-27.md`
5. `docs/decisions/CVF_CORE_REFRESH_TARGET_A0EF5923_ROOT_EFFECTS_2026-08-27.json`
6. `docs/decisions/CVF_CORE_REFRESH_TARGET_A0EF5923_WORKER_RETURN_2026-08-27.md`
7. `docs/decisions/CVF_CORE_REFRESH_TARGET_A0EF5923_COMPLETION_REVIEW_2026-08-27.md`

Paths 1-2 are the only mutable project carriers. Paths 3-7 are governance and
evidence artifacts owned by the roles above. No continuity, catalog,
Knowledge, status, roadmap, product source, test or P4-D path may change in
...
set for this refresh: capture their existence, status and SHA-256 preimages
without staging them; after worker and reviewer actions, prove those same 54
paths retain identical existence, status and bytes. Never edit, restore,
stage, commit or push any member of that P4-D set in this Core-refresh commit.
...
The initializer may rewrite ignored `.cvf/local-binding.json`. This binding,
the complete old hidden Core, and all 17 root targets require preimage and
postimage evidence. Before execution, preserve the binding's existence/bytes,
each root target's existence/bytes, and the complete old Core inside one fresh,
...
The current sanctioned scripts are frozen for authorization review:
...
- project `HEAD == origin/main ==
  a02a41d1a47b9251a3f70f94e2bff3b7bee017c2` and staged set empty;
...
- Core path resolves to the sibling hidden Core, Core tracked worktree/index
  are clean at `9c01832930226f2f770eafa346e01279160f22cb`, and origin is the
  exact public remote;
- local fetched `origin/main` and the available target object equal the full
  frozen target; if advertised `main` later differs, stop and roll back;
- DESIGN, SPEC, parent Work Order, reconciler and initializer hashes equal the
  frozen values above;
...
execution authority and runs no doctor, but any changed target requires fresh
...
project comparison, P4-D exact-54 preservation result, staged-zero state and
rollback result when applicable. Path 6 concisely links that receipt and
records the worker disposition and claim boundary. Plain transcripts remain in
the contained evidence directory.
...
evidence, then restore and verify in this order: old Core, all `17/17` root
targets, ignored local binding, paths 1-2, and all exact-54 P4-D preimages.
Retain paths 3-6 as the bounded governance/failure record. Run exactly one
rollback-verifier doctor only after restoration; record its result and stop.
Rollback cannot convert failure into success or permit a retry.
...
1. verify project `HEAD == origin/main == a02a41d1...`, staged set empty,
...
3. commit once with message
   `chore(cvf): refresh core to a0ef5923 (doctor pass)`;
...
this separate Core-refresh commit/push succeeds; this amendment does not itself
close, FREEZE or commit P4-D.

## 11. Claim boundary and disposition

PASS proves only that this workspace's public hidden Core and three project
pin carriers (manifest, ignored local binding and AGENTS header) are aligned to
the exact approved public target, with bounded root/project effects and doctor
