...
| data_scope | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20), chưa có application caller** | `cvf_runtime/data_scope.py · assert_placement_allowed`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 4/11, sau context admission) | `packages/ai-gateway` là pure library: `AIGateway.execute` là sole dispatch point, gọi hàm gốc `cvf_runtime` (không reimplement — xem `tests/unit/test_p4a_gateway_dependency_boundaries.py::test_gate_functions_are_the_cvf_runtime_originals`) trước mọi provider I/O. **Chưa có application/API caller nào gọi `AIGateway`** — request path FastAPI hiện tại không đụng tới. `allow_after_minimization` cho INTERNAL vẫn yêu cầu `minimization_proven=True` tường minh ở tầng gateway (P4-A1 hiện `NOT_PROVEN` nên bị refuse). Replacement live evidence sau repair: `docs/decisions/P4A_AI_GATEWAY_LIVE_EVIDENCE_RECEIPT.md`; independent final `REVIEW_PASS`. |
...
| cost | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20), chưa có application caller** | `cvf_runtime/budget.py · assert_within_budget`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 5/11, sau reserve trên process-local `UsageLedger`) | Reservation atomic (reserve→commit/release) trước khi gọi gate gốc; `fallback_to_rules` action trả về deterministic rules fallback (`ai_gateway.fallback`) thay vì provider call. Ledger chỉ process-local, không durable — chưa là accounting production. Application/API caller vẫn chưa tồn tại. |
...
| termination | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20), chưa có application caller** | `cvf_runtime/termination.py · assert_not_terminated`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 6/11, kill-switch/preflight trước dispatch) và lại tại timeout post-dispatch | Timeout dùng `asyncio.wait_for` + best-effort `cancel_request`, ghi nhận đúng một physical attempt, không retry. Application/API caller vẫn chưa tồn tại. |
