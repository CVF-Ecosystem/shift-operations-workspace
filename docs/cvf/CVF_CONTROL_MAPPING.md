...
| data_scope | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20); bounded application caller `CLOSED_BOUNDED` (P4-A2, 2026-08-21)** | `cvf_runtime/data_scope.py · assert_placement_allowed`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 4/11, sau context admission) | `packages/ai-gateway` là pure library: `AIGateway.execute` là sole dispatch point, gọi hàm gốc `cvf_runtime` (không reimplement — xem `tests/unit/test_p4a_gateway_dependency_boundaries.py::test_gate_functions_are_the_cvf_runtime_originals`) trước mọi provider I/O. `workspace_api.application.governed_rag.execute_governed_rag` (P4-A2) là first bounded application caller: nó gọi P4-A1 rồi `governed_rag.GovernedRAG.execute`, dispatch injected `AIGateway` tối đa một lần. **P4A2-REV-F3:** registry-owned `placement` bind tại construction từ real adapter wiring; replacement live evidence dùng `Placement.EXTERNAL`, chứng minh sáu refusal zero-call rồi đúng một HTTP 200 external call. `allow_after_minimization` cho INTERNAL vẫn yêu cầu `minimization_proven=True` tường minh ở tầng gateway. P4-A2 independent final `REVIEW_PASS`, findings/waivers `NONE/NONE`; receipt: `docs/decisions/P4A2_GOVERNED_RAG_LIVE_EVIDENCE_RECEIPT.md`. P4-A3 application-memory (2026-08-21) binds an authorization-scope digest as ADVISORY only (no `cvf_runtime` gate); see `docs/specs/P4A3_APPLICATION_MEMORY_SPEC.md`. |
...
| cost | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20); bounded application caller `CLOSED_BOUNDED` (P4-A2, 2026-08-21)** | `cvf_runtime/budget.py · assert_within_budget`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 5/11, sau reserve trên process-local `UsageLedger`) | Reservation atomic (reserve→commit/release) trước khi gọi gate gốc; `fallback_to_rules` action trả về deterministic rules fallback (`ai_gateway.fallback`) thay vì provider call. Ledger chỉ process-local, không durable — chưa là accounting production. `execute_governed_rag` injects a fresh `UsageLedger`; replacement live evidence recorded exactly one physical/adapter/gateway attempt. Durable/shared usage accounting vẫn ngoài phạm vi. |
...
| termination | callable, **reviewed real library caller (P4-A `CLOSED_BOUNDED`, 2026-08-20); bounded application caller `CLOSED_BOUNDED` (P4-A2, 2026-08-21)** | `cvf_runtime/termination.py · assert_not_terminated`, gọi trực tiếp bởi `ai_gateway.service.AIGateway.execute` (bước 6/11, kill-switch/preflight trước dispatch) và lại tại timeout post-dispatch | Timeout dùng `asyncio.wait_for` + best-effort `cancel_request`, ghi nhận đúng một physical attempt, không retry. P4-A2 không mở route API công khai; pipeline fail-closed zero-attempt trên P4-A1 negative/stale-index/injection/minimization/context-budget outcomes, được replacement live evidence xác nhận trước đúng một admitted call. |
...
- `REREV-F1 REFRESH_COMPLETION_STILL_FALSE`: `refresh()`/`refreshShifts()`
  ...
  → `conflict_resolved` mở khoá nhưng vẫn hiện thông điệp conflict để xem giá trị mới; thất bại → giữ khoá, nút refresh thủ công); `outcome_unknown`
...
**2026-08-02 P2-C C3c final repairs.** Round-4 sửa saved-but-unconfirmed thành locked/manual-refresh-only, không auto-retry. Independent final review sau đó tìm Report UI matrix sai và thiếu AC-29: sửa đúng `DRAFT=version+submit`, `IN_REVIEW=version`, `APPROVED/FROZEN=no operator mutation`, thêm four-row test; detached exact-parent `b17a8cb...` rehearsal PASS và cleanup path/registration hoàn tất. Codex không gọi Claude CLI/MCP/provider; receipt trả `READY_FOR_INDEPENDENT_P2C_C3C_BUILD_FINAL_RE_REVIEW`. **2026-08-02 P2-C C3d (supervisor closeout UI) — không thêm CVF `required_control` mới; không đụng backend/domain/ledger/database/migration/OpenAPI.** BUILD thêm `supervisorApi.ts`/`supervisorContracts.ts`/`useSupervisorData.ts` (staffing state tách biệt, 403 ẩn panel không suy diễn role client) cộng sáu component `features/supervisor-actions/*` (staffing assign/revoke, event confirm/correct dùng toàn bộ event collection chứ không chỉ confirmed projection, approval receipt cho đúng năm cặp POST kể cả `event.correct`, incident acknowledge, handover review/acknowledge, report approve/revoke-qua-successor, shift freeze chỉ gửi `expected_version`). `SupervisorActions` gate toàn bộ operational subtree theo `staffingAvailable` (server-derived thật từ 403 `GET /staffing/*`, không phải role-ranking client) để giữ đúng `operator-flow-accessibility.spec.ts` (zero supervisor controls cho operator) — không phải advisory-capability suppression (C3D-WO-REV-F1 chỉ cấm ẩn control vì thiếu `approval.create`), mà là gate theo authority đã verify qua route. Playwright mới (`supervisor-flow.spec.ts`, `supervisor-flow-accessibility.spec.ts`) chứng minh staffing self-revoke xoá stale operational state, refusal matrix (wrong-role/unassigned/stale-version/missing-approval/wrong-destination/frozen-parent/retired-override) qua backend thật. `run_c3d_web_evidence.py` tái dùng `run_c3c_web_evidence.run_harness` qua tham số `checkpoint`/`playwright_grep` mới (default giữ nguyên C3c). Live governance runner (`run_p2c_c3d_live_governance_evidence.py` + `_p2c_c3d_live_evidence_support.py`) chứng minh bảy refusal case observed zero provider call, rồi một genuine staffing-assigned closeout (self-assign → event confirm với receipt từ approver khác → incident acknowledge → cross-shift handover review/acknowledge → report approve với receipt riêng → shift close/freeze) verify qua fresh stored-shift read (`FROZEN`) và đủ actor-bound audit action trước khi gọi provider đúng một lần.
**2026-08-02 P2-D BUILD candidate — không thêm CVF `required_control`, backend route hay server idempotency.** Frontend chỉ stage ba transition CAS (`task`, `customer_request`, `incident`) khi `navigator.onLine === false` trước dispatch, trong namespace actor-bound 50 item/24 giờ với strict schema; online transport ambiguity không vào queue. Replay per-tab FIFO/fail-stop re-authorize qua backend, HTTP success + refresh fail là `applied_stale` không replay; conflict/5xx/ambiguous không tự retry hay rewrite CAS. Foreground `polling sync` dùng một refresh coordinator, không claim push/exactly-once/full-offline. Real Chromium/FastAPI chứng minh zero-POST offline, cleanup, conflict head block, actor khác qua polling, assignment-loss clearing, navigation-only cache và two-tab tối đa một CAS commit; live provider proof chỉ được ghi sau refusal/durability/PostgreSQL gates.
**2026-08-02 Phase 2 full-shift exit BUILD candidate — evidence composition only, no new `required_control` or product mutation.** One real Chromium/FastAPI lineage composes exact 12-hour scheduling, rendered staffing for distinct `sup1`/`sup2`, R0 event/task creation, pre-dispatch offline CAS replay, polling-observed event confirmation, immutable non-empty handover review/acknowledgement, close, END_SHIFT Report receipt/approval and atomic Shift+Report freeze. A separate disposable PostgreSQL 16 route/JWT test reconnects and verifies the exact ACTIVE operator/sup1/sup2 assignment set on both shifts (including automatic creator assignments), full ordered handover items, the six-field Report receipt scope plus exact approver identity, and an exact target/action/actor audit multiset. Every refusal compares a canonical whole-ledger fingerprint before/after while the provider counter remains zero. Browser evidence is producer-bound: the Playwright spec writes an owned run-id assertion artifact only after its checks pass; the wrapper enforces bounded asset sanitation before labeling PASS, binds producer output to the spec and harness SHA-256 digests, and provider admission recomputes those bindings and requires an exact committed task id/version/status from a successful post-replay GET before DOM rendering. The first physical provider call is retained but `INVALIDATED_BY_REVIEW_FAIL`; after independent pre-call `REVIEW_PASS`, the sole replacement was durably reserved before network, returned PASS/HTTP 200/exact token and was recorded `ACCEPTED`. Accounting is exactly two physical calls and one accepted final call; the persisted reservation/result blocks all reruns and the verified rerun exited 5 without a third call. Transport ambiguity still proves one request/no retry/no queue plus authoritative reconciliation; it does not claim the mutation failed to commit. The exact candidate received independent final post-call `REVIEW_PASS` with no finding or waiver and is pending its reviewed BUILD push plus separate C4; it does not itself close Phase 2 or claim wall-clock soak, push/exactly-once, fully-offline or production readiness. **2026-08-21 P4-B AI provider foundation — no new CVF `required_control`.** `packages/ai-providers`'s `ProviderModeService.execute` is the sole mode-selection entry point (zero-call `NO_AI`; deterministic local `RULES_ONLY` with construction-time ambiguity rejection, exact-type-checked scalar fact matching, and real `ai_gateway.validation` schema checking, never falling through on invalid output; default-denied `MockProviderAdapter` whose authorization is reconstructed and revalidated from its own primitive dump rather than trusted via `isinstance`; `EXTERNAL_AI` delegating exactly once to an injected P4-A `AIGateway`, zero-call on any outer/nested task/mode/schema/provider/model/placement/context-digest mismatch AND on the load-bearing `ProviderAdapterRegistry` check — an unregistered or `MOCK`-kind target is refused before delegation regardless of which P4-A `ProviderRegistry` the gateway itself holds). `execute` returns a strict `ProviderModeResultV1` envelope (releasable output bound to terminal outcome, digest-verified against the receipt, deep-copy isolated) instead of discarding successful output. Receipts enforce exact cross-field grammar per outcome and a closed `ai_mode` vocabulary kept mutually exhaustive with the published JSON schema. `workspace_api.application.ai_provider_modes` opens no route and persists nothing. An independent completion review (`REVIEW_FAIL / REPAIR_REQUIRED`, findings P4B-REV-F1..F6) preceded repair round 1, which made zero provider/network/install/database/commit/push/deployment calls; focused P4-B 203/203, P4-A/A2/A3 regression 698/698, full suite all pass. Status `READY_FOR_REREVIEW`; no production adapter, automatic routing, durable usage/audit, public API/UI or deployment claim.
