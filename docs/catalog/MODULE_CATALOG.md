...
_Last generated: 2026-07-30T11:25:51.372315+00:00_
...
|---|---|---|---:|---|---|
| `cvf-runtime` | packages/cvf-runtime | enforced | 876 | identity, permission, domain_lock, data_scope, risk, approval, evidence, audit, cost, refusal, termination, freeze | Runtime enforcement of the CVF application profile: reads the profile YAML and exposes all 12 required_controls as callable gates. |
| `operations-ledger` | packages/operations-ledger | enforced | 1838 | evidence, audit, freeze | Source-of-truth persistence. Defines the Ledger Protocol and an append-only, dual-backend SqlLedger (SQLAlchemy Core over the existing migration schema; generic Uuid/JSON types work against SQLite or PostgreSQL from the same table definitions). InMemoryLedger (in workspace-api) is the offline/test backend. |
...
- **Metrics:** 876 LOC across 13 code file(s)
...
- **Metrics:** 1838 LOC across 14 code file(s)
...
- **CVF controls:** —
- **Enforcement:** 2026-07-23 (P1B-OPERATIONS-DOMAIN-EXTRACTION): src/operations_domain/ is now the single canonical definition of twelve operational types (DataState, RiskClass, ShiftStatus, TaskStatus, CustomerRequestStatus, EvidenceRef, Shift, Message, OperationalEvent, Correction, Task, CustomerRequest - models.py) and their three lifecycle guards (assert_transition, assert_task_transition, assert_customer_request_transition - lifecycle.py), moved verbatim out of workspace-api. This package imports only the standard library and pydantic - never workspace_api, operations_ledger, cvf_runtime, fastapi or sqlalchemy - and tests/unit/test_operations_domain_boundary.py enforces that with a repository-scoped import-isolation subprocess plus AST scans, not just a docstring claim. workspace_api.domain.models / .lifecycle are now compatibility shims that re-export these same objects with proven object identity (tests/unit/test_operations_domain_shim_identity.py asserts `is`, not `==`, per module pair) rather than redefining them. User is the one documented exception: it stays defined in workspace_api.domain.models because it belongs to the authentication boundary (mirrors database/migrations/003_users.sql), not the operations domain - its eventual home is owned by the known-principals.yaml <-> users reconciliation tranche, not this one. Status is partial, not enforced: the eleven README-only blueprint subdirectories (approvals/, audit/, corrections/, customers/, events/, handovers/, incidents/, messages/, reports/, shifts/, tasks/) still have no code - only the models/lifecycle extraction is done. See docs/decisions/ADR_2026-07-23_P1B_OPERATIONS_DOMAIN_EXTRACTION.md and its catalog-gate addendum. 2026-07-26 (P2A-HANDOVER-VERTICAL): Handover, HandoverItem and HandoverStatus (DRAFT/REVIEWED/ACKNOWLEDGED) added to models.py, plus assert_handover_transition to lifecycle.py, mirroring the existing per-domain pattern exactly. Handover.acknowledged is a computed field derived from status (never independently settable) so it can never contradict it while still satisfying the pre-existing public contract field name.
- **Contract:** packages/workspace-contracts
- **Depends on:** `shared-kernel`
- **Tests:** `tests/unit/test_operations_domain_boundary.py`, `tests/unit/test_operations_domain_shim_identity.py`, `tests/unit/test_operations_domain_serialization.py`, `tests/cvf/test_handover_vertical.py`
- **Metrics:** 408 LOC across 3 code file(s)
...
- **Metrics:** 3807 LOC across 60 code file(s)
- **Next step:** P2-B real authentication reached FREEZE on 2026-07-23 after corrective REVIEW_PASS and live Alibaba evidence PASS (docs/decisions/P2B_IDENTITY_LIVE_EVIDENCE_RECEIPT.md). P1B-OPERATIONS-DOMAIN-EXTRACTION (2026-07-23) moved the operational models/lifecycle guards out to operations-domain; this app now consumes them as a dependency rather than owning their definition. P2A-HANDOVER-VERTICAL (2026-07-26) closed the handover domain. SHIFT-CREATE-ADMISSION-REPAIR (2026-07-29) and MESSAGE-ADMISSION-TRUST-REPAIR (2026-07-30) closed the previously-anonymous POST /shifts and internal POST /messages admission gaps. Next roadmap move is reports (P2-D), the frontend (P2-C), or a governed external/channel message-ingestion tranche (Integration Edge) - not another internal-admission repair.
...
- **Next step:** Open a fresh governed tranche for the parked unauthenticated shift-create mutation before adding any mutation UI; offline/realtime activation remains a separate P2-D tranche.
