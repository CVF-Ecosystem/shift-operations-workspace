"""Support module for run_report_live_governance_evidence.py.

P2R-OPERATIONAL-REPORT-FREEZE-PREREQUISITE (SPEC R31): owns everything that
touches provider HTTP, sanitization, safe endpoint description, provider-call
accounting and receipt rendering, mirroring the handover vertical's proven
design (_handover_live_evidence_support.py) but deliberately self-contained
rather than cross-imported, matching this repo's established convention of
...
class ProviderCallCounter:
    """A fresh instance per runner invocation (SPEC R31 parity) - never a
...
def render_receipt(
    ...
    lines = [
        ...
        "Produced by `scripts/run_report_live_governance_evidence.py` via "
        "`scripts/_report_live_evidence_support.py` "
        "(P2R-OPERATIONAL-REPORT-FREEZE-PREREQUISITE, SPEC R31). Sanitized: "
        "contains no API key, no Authorization header, no JWT, no raw "
        "secret, no URL userinfo/query/fragment.", "",
    ...
    lines += [
        ...
        "", "## Claim boundary", "",
        "This receipt evidences that a closed shift can be frozen only with "
        "a current, immutable, server-derived END_SHIFT Report whose exact "
        "version and snapshot digest have a valid R2 approval, atomically "
        "with Shift freeze and handover readiness, on the proven backends. "
        "It does NOT evidence report rendering/export, AI-generated "
        "operational truth, that any production endpoint calls a provider "
        "(none do), or PostgreSQL production readiness.", "",
    ]
