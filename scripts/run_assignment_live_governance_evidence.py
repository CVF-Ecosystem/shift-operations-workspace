#!/usr/bin/env python3
"""Live governance evidence for the assignment/staffing foundation
(P2C-MUTATION-FULL-UI-C3A1, SPEC R10). In-process refusal probes over the
real FastAPI/JWT route chain (zero provider calls each), then a genuine
durable staffing assignment admission, followed by exactly one real,
non-mocked provider call. Self-contained (no support module): the amended
C3a1 ceiling authorizes exactly this one new script path."""
...
PROMPT = "Reply with exactly this token and nothing else: CVF_ASSIGNMENT_EVIDENCE_OK"
EXPECTED_TOKEN = "CVF_ASSIGNMENT_EVIDENCE_OK"
RECEIPT_PATH = REPO_ROOT / "docs" / "decisions" / "P2C_C3A1_ASSIGNMENT_LIVE_EVIDENCE_RECEIPT.md"
...
def _evidence_shift(ledger, creator_id="assign-ev-sup"):
    from cvf_runtime.identity import Principal
    ...
    return ShiftService(ledger).create(
        "Assignment evidence shift", starts_at, ends_at, Principal(user_id=creator_id, role="shift_supervisor")
    )

_REFUSAL_CASES = (
    ("anonymous_staffing_list_rejected", lambda c, sid: c.get("/staffing/shifts"), 401),
    ("operator_cannot_manage_staffing", lambda c, sid: c.post(
        f"/shifts/{sid}/assignments", json={"user_id": "assign-ev-op"}, headers=_auth_headers("assign-ev-op", "operator")
    ), 403),
    ("capabilities_denied_without_active_assignment", lambda c, sid: c.get(
        f"/shifts/{sid}/capabilities", headers=_auth_headers("assign-ev-op", "operator")
    ), 403),
)

def check_assignment_refusal_gate(counter: ProviderCallCounter) -> list[dict]:
    """Refusal cases: each records the OBSERVED provider-call delta (SPEC
    R10), never a hard-coded literal."""
    from workspace_api.infrastructure.repository import InMemoryLedger

    results: list[dict] = []
    for name, run, expected_status in _REFUSAL_CASES:
        ledger = InMemoryLedger()
        _seed(ledger, "assign-ev-sup", "shift_supervisor")
        _seed(ledger, "assign-ev-op", "operator")
        shift = _evidence_shift(ledger)
...
def build_admitted_staffing_genuine() -> tuple[bool, str]:
    """Genuine durable staffing assignment via a minted supervisor JWT and a
    real HTTP request; asserts exactly one ACTIVE assignment plus one audit."""
    from workspace_api.infrastructure.repository import InMemoryLedger

    ledger = InMemoryLedger()
    _seed(ledger, "assign-ev-sup", "shift_supervisor")
    _seed(ledger, "assign-ev-target", "operator")
    shift = _evidence_shift(ledger)
    headers = _auth_headers("assign-ev-sup", "shift_supervisor")

    def _run(client):
        res = client.post(f"/shifts/{shift.shift_id}/assignments", json={"user_id": "assign-ev-target"}, headers=headers)
        if res.status_code != 200:
            return False, f"assign failed: {res.status_code}"
        body = res.json()
        active = ledger.get_active_assignment(shift.shift_id, "assign-ev-target")
        if active is None or str(active.assignment_id) != body["assignment_id"]:
            return False, "expected exactly one durable ACTIVE assignment matching the response"
        audits = ledger.audit_entries_for(body["assignment_id"])
        if len(audits) != 1:
            return False, f"expected exactly one audit for the created assignment, found {len(audits)}"
        a = audits[0]
        chain = ["identity", "permission", "assign", "audit"]
        expected = {"actor_id": "assign-ev-sup", "actor_role": "shift_supervisor", "action": "shift.assignment.manage",
                    "record_type": "ShiftAssignment", "record_id": body["assignment_id"], "control_chain": chain,
                    "before_state": None, "after_state": "ACTIVE"}
        ...
        return True, (
            "valid supervisor JWT (assign-ev-sup) admitted staffing assignment via a minted token and a real "
            "HTTP request, persisting exactly one durable ACTIVE assignment and one exactly-field-matched audit"
        )
...
def _endpoint() -> str:
    base_url = next(
        (os.environ[n].strip() for n in BASE_URL_ENV_NAMES if os.environ.get(n, "").strip()), DEFAULT_BASE_URL
    ).rstrip("/")
...
def render_receipt(path, *, gate_results, admitted_detail, provider_result, model, safe_endpoint, call_count) -> None:
    ...
    lines = [
        ...
        "Produced by `scripts/run_assignment_live_governance_evidence.py` (P2C-MUTATION-FULL-UI-C3A1, SPEC R10). "
        "Sanitized: no API key, Authorization header, JWT, raw secret, or URL userinfo/query/fragment.", "",
    ...
    lines += [
        "", "## 2. Genuine durable staffing assignment", "", f"- {admitted_detail}", "",
    ...
    lines += [
        ...
        "## Claim boundary", "",
        "Evidences that the staffing control plane's real identity/permission/assignment gate correctly refuses "
        "anonymous, insufficient-role and unassigned-capability attempts before any provider call, and correctly "
        "admits a genuine valid-supervisor-JWT staffing assignment through the real HTTP route chain, persisting "
        "exactly one durable ACTIVE assignment and one actor-bound audit record. Does NOT evidence that C3a1 "
        "enforces assignment across existing operational routes (C3a2), production PostgreSQL readiness, or "
        "frontend mutation.", "",
    ]
    path.write_text("\n".join(lines), encoding="utf-8")

def main() -> int:
    parser = argparse.ArgumentParser(description="Assignment/staffing live governance evidence")
    parser.add_argument("--dry-run", action="store_true", help="run without calling provider")
    args = parser.parse_args()
    counter = ProviderCallCounter()

    print("== assignment/staffing refusal gate: refusal cases ==")
    gate_results = check_assignment_refusal_gate(counter)
    for r in gate_results:
        print(f"  {r['case']}: {r['outcome']} - {r['detail']}")
    if any(r["outcome"] != "PASS" for r in gate_results) or any(r["calls"] != 0 for r in gate_results):
        print("ASSIGNMENT REFUSAL GATE CASES FAILED", file=sys.stderr)
        return 1

    print("== constructing a genuine durable staffing assignment ==")
    ok, detail = build_admitted_staffing_genuine()
    print(f"  {detail}")
    if not ok:
        print("GENUINE ASSIGNMENT CONSTRUCTION FAILED", file=sys.stderr)
        return 1
