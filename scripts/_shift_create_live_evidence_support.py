"""Support module for run_shift_create_live_governance_evidence.py.

SHIFT-CREATE-ADMISSION-REPAIR-2026-07-29 (SPEC R12-R14): owns everything that
touches provider HTTP, sanitization, safe endpoint description, provider-call
accounting and receipt rendering, mirroring the incident/handover verticals'
proven design (_incident_live_evidence_support.py,
_handover_live_evidence_support.py) but deliberately self-contained rather
than cross-imported, per this repo's established convention. Nothing here
ever returns, prints, or writes the exact configured provider key, a
bearer/JWT value, or a URL's userinfo/query/fragment - every provider-derived
string is sanitized before it leaves :func:`call_provider`.
"""
...
class ProviderCallCounter:
    """A fresh instance per runner invocation (SPEC R12 parity) - never a
...
def render_receipt(
    ...
    lines = [
        ...
        "Produced by `scripts/run_shift_create_live_governance_evidence.py` via "
        "`scripts/_shift_create_live_evidence_support.py` "
        "(SHIFT-CREATE-ADMISSION-REPAIR-2026-07-29, SPEC R12-R14). Sanitized: "
    ...
    lines += [
        ...
        "operator JWT and persisted the shift plus its actor-bound audit.", "",
    ...
    lines += [
        ...
        "gate correctly refuses anonymous, malformed-token, insufficient-role "
        "and invalid-window create attempts before any provider call, and "
        "correctly admits a genuine valid-operator-JWT create through the "
        "real HTTP route chain, persisting exactly one shift and one "
        "actor-bound audit record. It does NOT evidence that the production "
        "POST /shifts endpoint calls a provider in production (it does not), "
        "does not evidence message-admission identity, and does not evidence "
