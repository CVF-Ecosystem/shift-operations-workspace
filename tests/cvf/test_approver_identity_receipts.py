...
def test_ac23_r3_confirm_passes_for_every_receipt_creation_order(order):
    ...
    assert EventService(ledger, audit=None).confirm(
...
def test_f15_confirmer_plus_distinct_valid_r2_approver_succeeds():
    ...
    assert svc.confirm(event.event_id, _confirmer(), expected_version=event.version).state == DataState.CONFIRMED
...
def test_f15_sole_confirmer_cannot_self_approve_fails():
    ...
    svc = EventService(ledger, audit=None)
    with pytest.raises(CvfDenied) as exc:
        svc.confirm(event.event_id, _confirmer(), expected_version=event.version)
...
def test_f18b_r3_permutations_http_both_backends(tmp_path, name, order):
    ...
    try:
        ...
        assert client.post(f"/events/{event.event_id}/confirm", json={"expected_version": event.version}, headers=auth_headers("sup1", "shift_supervisor")).status_code == 200
    finally: _clear_overrides()
