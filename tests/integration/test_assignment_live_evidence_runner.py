"""Non-live tests for run_assignment_live_governance_evidence.py
(P2C-MUTATION-FULL-UI-C3A1, SPEC R10). None of these call a real provider or
need Docker/PostgreSQL - every case exercises the real in-process gate/HTTP
code path or a monkeypatched transport, asserting refusal cases make an
OBSERVED zero provider calls (not a hard-coded literal) and that no sentinel
secret ever reaches a returned summary, stdout/stderr, or the receipt file.
scripts/ is not on pytest's pythonpath, added here like every other
script-importing test module in this repo.
"""
...
import run_assignment_live_governance_evidence as runner  # noqa: E402
...
def test_refusal_gate_cases_all_pass_with_observed_zero_calls():
    ...
    results = runner.check_assignment_refusal_gate(counter)
    assert {r["case"] for r in results} == {
        "anonymous_staffing_list_rejected",
        "operator_cannot_manage_staffing",
        "capabilities_denied_without_active_assignment",
    }
    for r in results:
...
def test_genuine_admitted_staffing_construction_succeeds():
    ok, detail = runner.build_admitted_staffing_genuine()
    assert ok, detail
    assert "supervisor" in detail.lower() or "assign-ev-sup" in detail
...
def test_admitted_construction_rejects_a_mismatched_assignment_id(monkeypatch):
    """If get_active_assignment ever returned a DIFFERENT assignment than the
    one the HTTP response reported, the proof must fail rather than silently
    trust the response body alone."""
    from workspace_api.infrastructure.repository import InMemoryLedger

    original_get_active = InMemoryLedger.get_active_assignment

    def _swap_id(self, shift_id, user_id, *, unit=None):
        real = original_get_active(self, shift_id, user_id, unit=unit)
        if real is not None:
            real.assignment_id = __import__("uuid").uuid4()
        return real

    monkeypatch.setattr(InMemoryLedger, "get_active_assignment", _swap_id)
    ok, detail = runner.build_admitted_staffing_genuine()
    assert ok is False
    assert "matching the response" in detail
...
def test_write_receipt_is_sanitized(tmp_path, monkeypatch):
    ...
    provider_result = {"outcome": "PASS", "reached_server": True, "http_status": 200, "response_excerpt": "CVF_ASSIGNMENT_EVIDENCE_OK"}
    runner.render_receipt(
