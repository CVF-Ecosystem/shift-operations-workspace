...
2. Resolve and verify the core path. On a fresh clone, run
   ...
   to clone/pin the core and create the git-ignored local binding.
...
6. `AGENT_HANDOFF.md`, resolved from `cvfCoreRelativePath` in `.cvf/manifest.json`
   ...
   the portable core path from the manifest first.
7. `AGENTS.md`, resolved relative to the same portable core path
