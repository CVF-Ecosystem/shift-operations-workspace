...
``approvals`` list is retired; the server auto-collects persisted,
authenticated receipts matching the event's current scope (SPEC R5.1) - see
``services.py``'s module docstring for the shared rationale.
"""
    ...
    def correct_event(
        ...
        *,
        ...
        with self.ledger.transaction() as unit:
            ...
            # CURRENT scope (pre-correction version), evaluated fresh.
            ...
            # Record the correction (before/after preserved as versions). The
            # frozen record is not silently overwritten: state stays FROZEN
            # if it was frozen; a CONFIRMED record moves to CORRECTED.
            ...
            # allow_when_frozen=True: a correction record is the one
            # permitted post-freeze mutation path (freeze-policy.yaml:
            # post_freeze_mutation = correction_record_only). Every other
            # write path defaults to blocking a frozen shift.
