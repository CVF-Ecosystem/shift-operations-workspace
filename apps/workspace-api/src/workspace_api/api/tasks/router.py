...
def list_tasks(
    shift_id: UUID = Query(..., description="Filter tasks by shift"),
    principal: Principal = Depends(get_principal),
    ledger: Ledger = Depends(get_ledger),
):
    """P2C-MUTATION-FULL-UI-C3B1 (SPEC R11/R36/R37): authenticated,
    ACTIVE-assignment-scoped Task list including terminal history, ascending
    (created_at, task_id). 501+ matches return controlled 422."""
    try:
        return browser_reads.list_tasks_for_shift(ledger, shift_id, principal)
    except CvfDenied as exc:
        raise HTTPException(status_code=exc.http_status, detail=str(exc)) from exc
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Operational resource not found") from exc
